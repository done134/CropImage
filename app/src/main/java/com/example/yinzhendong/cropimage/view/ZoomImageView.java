...

{
	...
	public void onGlobalLayout()
	{
		...
		if (preferences.getLong(d.toString(), -1) != 1){
			...
			if (!isCrop){
				...
				if (dw >= getWidth() - mHorizontalPadding * 2
						&& dh >= getHeight() - mVerticalPadding * 2)
				{
					float scaleW = dw / (getWidth() * 1.0f - mHorizontalPadding * 2);
					float scaleH = dh / (getHeight() * 1.0f - mVerticalPadding * 2);
			...
			// 图片移动至屏幕中心
			setImageMatrix(mScaleMatrix);
			preferences.edit().putLong(d.toString(), 1).commit();
		}
